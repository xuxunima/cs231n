...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        ############################################################################
        C, H, W = input_dim
        out_size = int(num_filters * H * W / 4)
        W1 = np.random.randn(num_filters,C, filter_size,filter_size) * weight_scale
        b1 = np.zeros(num_filters)
        W2 = np.random.randn(out_size, hidden_dim) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        h_out, cache_h = affine_relu_forward(conv_out, W2, b2)
        scores, cache_s = affine_forward(h_out, W3, b3)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dx, dW3, db3 = affine_backward(dx, cache_s)
        dx, dW2, db2 = affine_relu_backward(dx, cache_h)
        
        _, dW1, db1 = conv_relu_pool_backward(dx, cache_conv)
